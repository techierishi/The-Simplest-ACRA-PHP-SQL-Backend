...
function initializeSQL($file, $device_id, $android_ver, $app_version, $time, $LOG, $package){
    ...
    $lookForIssueId = mysqli_query($mysql, $getIssueID);
    if(!$lookForIssueId) die('failed! ' . mysqli_error() . '<br>');
    else echo 'Success! Issue ID loaded<br>';
    
    $count = mysqli_num_rows($lookForIssueId);
    
    if($count > 0){
        echo $count . 'exists. Replacing values newest_report and recorded_events<br>';
        $row = mysqli_fetch_assoc($lookForIssueId);
      
            $newestReport = $time;
            $recorded_events = intval($row['recorded_events']);
            $recorded_events += 1;
            $device_ids = $row[''];
            echo $recorded_events . ' events reported<br>';
            echo '     ' . $newestReport;
            $android_version = $row['android_version'];
            if(strpos($android_ver, $android_version) === false){
                $android_version .= ', ' . $android_ver;//adds the android version of the new device that reported the error
            }
            if(strpos($device_id, $device_ids) === false){
                $device_ids .= ', ' . $device_id;//adds the model of the new phone to the list of phones
            }
            $result = mysqli_query($mysql, "UPDATE exceptions SET newest_report='$newestReport', recorded_events='$recorded_events', device_id='$device_ids', android_version='$android_version' WHERE issue_id = '$issue_id'");
